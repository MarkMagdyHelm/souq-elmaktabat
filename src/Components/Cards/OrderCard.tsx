...
type Props = {
  item: any, onAccept: any, onReject: any, onPress: any
}
        ...
        <View style={[layout.dirRow, styles.row]}>
          <View style={styles.con1}>
            ...
            <View style={[layout.rowBox, styles.actions]}>
              <Text style={styles.quantity}>{t("quantity")} {item.quentity} {t("carton")}</Text>
              <Text style={styles.price}>{item.price} {t("pound")}</Text>
            </View>

            <Text style={styles.price}>{t("governorateArea") + " " + item.branch}</Text>
...
const useStyles = (Fonts: IFont, theme: ITheme, darkmode: boolean, dir: string,) =>
  StyleSheet.create({
    ...
    quantity: { textAlign: "right", fontSize: PixelPerfect(14), color: theme.black, fontFamily: Fonts.medium },
    con1: { flex: 1, marginHorizontal: PixelPerfect(8) },
    price: { textAlign: "right", fontSize: PixelPerfect(14), color: theme.textColor, fontFamily: Fonts.bold },
