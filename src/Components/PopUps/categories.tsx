import { FlatList, Image, Pressable, StyleSheet, Text, View, ViewStyle } from 'react-native'
        ...
        >
            <View style={[styles.con, style]}>
                ...
                <FlatList
                    ...
                    style={{ paddingHorizontal: PixelPerfect(16),paddingTop:PixelPerfect(5) }}
                    renderItem={({ item }) => (
                        <Pressable
                            style={[layout.rowBox, styles.conitem]}
                            onPress={() => {
                                   onCloseFn && onCloseFn()
                                navigation.navigate("AddOffer", { item: item,type:item.name }); }}
                        >
                            <Image
                                source={{ uri: item.image }}
                                style={styles.img}
                            />
                            <Text style={[layout.textAlign,styles.itemtxt]}>{dir=="rtl"?item.arName:item.name}</Text>
                        </Pressable>
                    )}
                    ItemSeparatorComponent={() => <View style={{ paddingVertical: PixelPerfect(8) }} />}
                />
            </View>
        </Modal>
    )
