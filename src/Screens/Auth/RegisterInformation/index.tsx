...
const Index = (props: Props) => {
    ...
    const handleBody = () => {
        ...
        if (state.IsRorleCustommer) {
            ...
            body.Activities = formikRef2?.current?.values?.Activities.filter(item => item.id !== undefined)?.map(item => item.id) || [];
            ...
            body.Description = ""
            body.UserImageUrl = formikRef2?.current?.values?.ImageUrl;
            body.CompanyName = ""
        } else {
            body.OtherPhoneNumbers = state.otherPhones;
            body.Activities = formikRef3?.current?.values?.Activities.filter(item => item.id !== undefined)?.map(item => item.id) || [];
            body.OtherActivities = formikRef3?.current?.values?.Activities.filter(item => item.id === undefined) || [];
            body.AvailableTools = formikRef3?.current?.values?.AvailableTools.filter(item => item.id !== undefined)?.map(item => item.id) || [];
            body.OtherAvailebleTools = formikRef3?.current?.values?.AvailableTools.filter(item => item.id === undefined) || [];
            body.PaymentTypes = formikRef3?.current?.values?.PaymentTypes.filter(item => item.id !== undefined)?.map(item => item.id) || [];
