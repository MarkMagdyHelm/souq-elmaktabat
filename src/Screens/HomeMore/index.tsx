...
import { GetAllInkOffers, GetAllPaperOffers, GetAllPrintersOffers, GetPapersHandler } from '../../Apis/HomeApis';
...
const Index = (props: Props) => {
    ...
    const getAllInkOffers = (page: number = 1, loadMore: boolean = false) => {
        if (loadMore) {
            setstate(old => ({ ...old, loadingMore: true }));
        } else {
            setstate(old => ({ ...old, loading: true }));
    ...
    const getAllPrinting = (page: number = 1, loadMore: boolean = false) => {
        if (loadMore) {
            setstate(old => ({ ...old, loadingMore: true }));
        } else {
            setstate(old => ({ ...old, loading: true }));
        ...
        dispatch<any>(
            GetAllPrintersOffers({ page: page, pageSize: "10" }, (res) => {
                  if (res.status === 200) {
    ...
    return (
        <Container showHint={false}>
            <HeaderWithText title={sectionId == 1 ? t("paperOffers") : sectionId == 2 ? t("offerInks") : t("offerPrinters")} />
            <View style={{ flex: 1, paddingHorizontal: PixelPerfect(16) }}>
                ...
                <FlatList
                    ...
                    renderItem={({ item }) => <Product isOfffer={false} item={item} onPress={() => {
